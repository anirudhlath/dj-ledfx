...
def test_a_route_reads_its_slice_of_the_nearest_frame() -> None:
    ...
    route = DeviceRoute(ring=ring, start=1, stop=3, streaming=True)
...
def test_a_route_never_reads_past_the_end_of_a_frame() -> None:
    ...
    too_long = DeviceRoute(ring=ring, start=1, stop=3, streaming=True)
    assert too_long.colors_at(1.0, 2) is None
    empty = DeviceRoute(ring=RingBuffer(4), start=0, stop=2, streaming=True)
