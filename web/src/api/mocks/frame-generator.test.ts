...
describe('paint', () => {
  it.each(MOTIFS)('%s paints the LEDs and moves over time', (motif) => {
    ...
    paint(before, spec(motif), 0.3, 0.1, 1, 1)
    paint(after, spec(motif), 1.7, 0.6, 1, 1)
  ...
  it('paints the same for the same inputs', () => {
    ...
    paint(one, spec('ripple'), 2.5, 0.4, 0.8, 7)
    paint(two, spec('ripple'), 2.5, 0.4, 0.8, 7)
  ...
  it('scales with brightness, and paints black at 0', () => {
    ...
    paint(full, spec('glow'), 1, 0, 1, 0)
    paint(half, spec('glow'), 1, 0, 0.5, 0)
    paint(off, spec('glow'), 1, 0, 0, 0)
  ...
  it('follows the beat: a pulse is brightest on it', () => {
    ...
    paint(on, spec('pulse'), 1, 0, 1, 0)
    paint(late, spec('pulse'), 1, 0.9, 1, 0)
    expect(Math.max(...on)).toBeGreaterThan(Math.max(...late))
  })

  it('writes only its own LEDs, and takes a light with none', () => {
    const out = new Uint8Array(12)
    paint(out.subarray(0, 6), spec('gradient'), 1, 0, 1, 0)
    expect([...out.slice(6)]).toEqual([0, 0, 0, 0, 0, 0])
    expect(() => paint(new Uint8Array(0), spec('chase'), 1, 0, 1, 0)).not.toThrow()
  })
})
